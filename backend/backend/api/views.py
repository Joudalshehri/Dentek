...
def get_patient(request, patient_id):
    ...
    # Return error if patient does not exist
...
def get_xray_analysis(request, xray_id):
    ...
    """
    Retrieve the AI analysis result for a specific X-ray.
    Only the owner of the patient can access this analysis.
    """
    ...
    # Get the X-ray only if it belongs to the logged-in user's patient
    try:
        xray = XRay.objects.select_related("patient").get(
            id=xray_id,
            patient__user=request.user
    ...
    # Check if analysis exists for this X-ray
    if not xray.analysis_result:
        return Response(
            {"error": "No analysis found for this xray"},
            status=status.HTTP_404_NOT_FOUND
        )

    # Copy analysis result before adding extra fields
    data = xray.analysis_result.copy()

    # Add doctor notes and edited report
